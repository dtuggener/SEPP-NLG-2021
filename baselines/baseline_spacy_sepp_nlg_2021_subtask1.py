...
def predict_sent_end(data_zip: str, lang: str, data_set: str, outdir: str, overwrite: bool = False,
    ...
    if model_size in {'sm', 'md'}:
        if lang == 'en':
            spacy_model_name = f'en_core_web_{model_size}'
        elif lang in {'de', 'fr', 'it'}:
            spacy_model_name = f'{lang}_core_news_{model_size}'
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("model_size", default='sm', help="spaCy model size (sm, md, or lg)", nargs='?')
