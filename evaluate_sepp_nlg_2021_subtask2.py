    ...
    with ZipFile(data_zip, 'r') as zf:
        ...
        for i, gt_tsv_file in enumerate(gt_tsv_files, 1):
            ...
            if os.path.exists(prediction_file):
                ...
                if not len(gt_labels) == len(pred_labels):
                    print(f'\nWARNING: unequal no. of labels for files {gt_tsv_file} and {os.path.join(prediction_dir,  lang, data_set, basename)}')
                    continue
                all_gt_labels.extend(gt_labels)
                all_predicted_labels.extend(pred_labels)
            else:
                print('\nWARNING: gold file', prediction_file, 'does not exists in predictions')
        ...
        disp.plot()
        plt.savefig(f'{lang}_{data_set}_confusion_matrix.png')
